import React from "react";
import PageHeader from "../common/pageHeader";
import { Navigate } from "react-router-dom";
import {getProfilePic, getUsers} from "../../services/userService";
import UserExtends from "../common/users/userExtends";
import UserDisplayControllers from "../common/users/userDisplayControllers";
import UserDisplayModes from "../common/users/userDisplayModes";

class UserManagementInterface extends UserExtends {
  state = {
    users: [],
    isMount: false,
    display: this.getUserDisplay(),
    imageArray: [],
  };
  
  async componentDidMount() {
    try {
      const defaultUserImageUrl = "data:image/png;base64,";
      const { data } = await getUsers();
      const numUsers = data.length;
      const imageArray = [];
      for(let i=0;i<numUsers;i++){
        const blob = await getProfilePic(data[i]._id);
        if(!blob.data){
          imageArray[i] = {
            userId: data[i]._id,
            image: defaultUserImageUrl,
          }
        }
        else{
          imageArray[i] = {
            userId: data[i]._id,
            image: "data:image/png;base64,"+blob.data,
          }
        }
      }

      this.setState({ users: data, isMount: true ,imageArray});
    } catch (error) {
      console.log(error.message);
    }
  }

  render() {
    const { user } = this.props;
    if (!user.isAdmin) return <Navigate replace to="/" />;
    const users = [...this.state.users];
    const { isMount , display} = this.state;
    if (!isMount) return null;

    return (
      <React.Fragment>
        <PageHeader
          title="User Management Interface"
          subTitle="Here you can manage your users"
        />
        <div className="container">
          <UserDisplayControllers
          display={display}
          handleDisplay={this.handleDisplay}
          />
        <div>
          <UserDisplayModes
          users={users}
          handleDelete={this.handleDelete}
          makeAdmin= {this.makeAdmin}
          display={display}
          imageArray={this.state.imageArray}
        />
          </div>
        </div>
      </React.Fragment>
    );
  }
}

export default UserManagementInterface;
